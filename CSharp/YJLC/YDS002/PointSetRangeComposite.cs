...
class PointSetRangeComposite
{
	...
	static void Main()
	{
		...
		var h = Read();
		int n = h[0], qc = h[1];
		var f = Array.ConvertAll(new bool[n], _ => ToTuple2(ReadL()));
		var qs = Array.ConvertAll(new bool[qc], _ => Read());

		var st = new ST1<(long a, long b)>(n, (f1, f2) => (f1.a * f2.a % M, (f1.b * f2.a + f2.b) % M), (1, 0), f);
		...
		foreach (var q in qs)
		{
			...
			{
				var (a, b) = st.Get(q[1], q[2]);
				Console.WriteLine((a * q[3] + b) % M);
			}
		}
		Console.Out.Flush();
	}
