...
namespace CoderLib8.Numerics
{
	static class DftD
	{
		public static int[] ToInt(this ComplexD[] a) => Array.ConvertAll(a, c => (int)Math.Round(c.X));
		public static long[] ToLong(this ComplexD[] a) => Array.ConvertAll(a, c => (long)Math.Round(c.X));
		public static decimal[] ToDecimal(this ComplexD[] a) => Array.ConvertAll(a, c => c.X);
		...
		public static decimal[] Convolution(long[] a, long[] b)
		{
			...
			return Array.ConvertAll(Convolution(ac, bc), c => decimal.Round(c.X) % M);
		}
	}
