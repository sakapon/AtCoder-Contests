...
class N
{
	...
	static object Solve()
	{
		...
		for (int i = 0; i < xMap.Count; i++)
		{
			if (i > 0)
			{
				r += lst.Get(0, yMap.Count) * (xMap.ReverseMap[i] - xMap.ReverseMap[i - 1]);
			}

			if (!qd.ContainsKey(xMap.ReverseMap[i])) continue;

			foreach (var (add, _, b, d) in qd[xMap.ReverseMap[i]])
			{
				lst.Set(yMap[b], yMap[d], true);
			}
		}
		return r;
	}
